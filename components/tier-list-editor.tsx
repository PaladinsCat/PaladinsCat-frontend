...
export default function TierListEditor({
  ...
  async function submit(event: FormEvent) {
    ...
    setSaving(true);
    try {
      const input = { title: title.trim(), description: description.trim(), entries, token };
  ...
  const championCard = (championId: number, group: GroupKey) => {
    ...
    return <div key={champion.id} draggable onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("application/x-paladinscat-champion", String(champion.id)); }} className="group relative flex w-16 cursor-grab flex-col items-center rounded-lg border border-pc-border bg-pc-bg p-1.5 active:cursor-grabbing sm:w-20">
      ...
      <span className="mt-1 w-full truncate text-center text-xs text-pc-text-secondary">{champion.name}</span>
      <select aria-label={t("tierLists.moveChampion", { name: champion.name })} value={group === "tray" ? "" : group} onChange={(event) => moveChampion(champion.id, (event.target.value || "tray") as GroupKey)} className="mt-1 w-full rounded border border-pc-border bg-pc-bg-secondary px-0.5 py-0.5 text-xs text-pc-text sm:hidden">
