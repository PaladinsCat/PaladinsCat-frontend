...
function groupsFromEntries(champions: Champion[], entries: TierListEntry[] = []): Groups {
  const groups = emptyGroups();
  const currentChampionIds = new Set(champions.map((champion) => champion.id));
  const placed = new Set<number>();
  for (const tier of TIER_ORDER) {
    groups[tier] = entries
      .filter((entry) => entry.tier === tier && currentChampionIds.has(entry.championId))
      .sort((a, b) => a.position - b.position)
      .map((entry) => entry.championId)
      .filter((championId) => {
        if (placed.has(championId)) return false;
        placed.add(championId);
        return true;
      });
  }
  groups.tray = champions.map((champion) => champion.id).filter((id) => !placed.has(id));
  return groups;
}
...
export default function TierListEditor({
  ...
  useEffect(() => {
    ...
    fetchChampions({ limit: "200" })
      .then(async (rows) => {
        ...
        setGroups(groupsFromEntries(sorted, initialList?.entries));
  ...
  function moveChampion(championId: number, target: GroupKey) {
    setGroups((current) => {
      ...
      for (const group of [...TIER_ORDER, "tray"] as GroupKey[]) {
        next[group] = current[group].filter((id) => id !== championId);
      }
  ...
  function reset() {
    setGroups(groupsFromEntries(champions, initialList?.entries));
  }
  ...
  async function submit(event: FormEvent) {
    ...
    const entries = TIER_ORDER.flatMap((tier) => groups[tier].map((championId, position) => ({
      championId,
      tier,
      position,
    })));
  ...
  return <form onSubmit={submit} className="space-y-6">
    ...
    <div className="space-y-4 rounded-xl border border-pc-border bg-pc-bg-elevated p-4">
      <label className="block text-xs text-pc-text-secondary">{t("tierLists.mode")}<select value={mode} onChange={(event) => setMode(event.target.value as TierListMode)} className="mt-1.5 w-full rounded-lg border border-pc-border bg-pc-bg-secondary px-3 py-2 text-sm text-pc-text sm:max-w-xs"><option value="champion">{t("tierLists.championMode")}</option><option value="ultimate">{t("tierLists.ultimateMode")}</option></select></label>
