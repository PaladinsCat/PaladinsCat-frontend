...
export default function PlayerTrendsPanel({ playerId, champions = false }: { playerId: string; champions?: boolean }) {
  ...
  return <section className="pc-card space-y-4" aria-labelledby={titleId}>
    ...
    {loading?<LoadingIndicator />:error?<ErrorState title={t("playerTrends.failed")} onRetry={()=>setRetry(value=>value+1)} />:!hasData?<EmptyState title={t("playerTrends.empty")} />:<div className="min-w-0 space-y-3">
      <LineChartComponent data={chart} xKey="date" yKeys={scopes.map(name)} height={300} percentageScale={metric==="winRate"} showLegend={false} />
    ...
    {!loading&&!error&&data&&metric!=="elo"&&<div className="overflow-x-auto"><table className="w-full whitespace-nowrap text-sm"><thead><tr className="text-left text-xs text-pc-text-secondary"><th className="py-2">{t("playerTrends.champions")}</th><th>{t("playerTrends.current")}</th><th>{t("playerTrends.change")}</th><th>{t("playerTrends.global")}</th><th>{t("playerTrends.samples")}</th></tr></thead><tbody>
      {scopes.map(id=>{const total=data.totals.find(row=>row.championId===id);const current=value(total);const before=total?value(openingTrendTotals(total,data.buckets)):null;const global=value(data.globalTotals.find(row=>row.championId===id));const fmt=(v:number|null)=>v==null?"—":formatNumber(v,{maximumFractionDigits:2});return <tr key={id} className="border-t border-pc-border"><th className="py-2 text-left font-medium">{name(id)}</th><td>{fmt(current)}</td><td>{current==null||before==null?"—":formatNumber(current-before,{maximumFractionDigits:2,signDisplay:"always"})}</td><td>{fmt(global)}</td><td>{formatNumber(metric==="winRate"?total?.matches??0:total?.samples[metric]??0)}</td></tr>;})}
    </tbody></table></div>}
  </section>;
}
