...
function PolicySectionLink({ link }: { link: NonNullable<PublicPolicySection["link"]> }) {
  const external = /^https?:\/\//i.test(link.href);
  const LinkIcon = external ? ArrowUpRight : ArrowRight;
  return <>{" "}<a href={link.href} {...(external ? { target: "_blank", rel: "noreferrer noopener" } : {})} className="inline-flex items-center gap-1 font-semibold text-pc-accent hover:text-pc-accent-light">{link.label}<LinkIcon className="h-3.5 w-3.5" aria-hidden="true" /></a>.</>;
}
...
export default function PublicPolicyPage({
  ...
  return (
    <div className="pc-glass mx-auto max-w-6xl overflow-hidden rounded-2xl border border-white/5 shadow-lg">
      <header className="relative overflow-hidden border-b border-pc-border px-5 py-12 sm:px-10 sm:py-16 lg:px-16">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          <div className="max-w-3xl">
            ...
            <h1 className="pc-heading pc-heading-lg mt-4">{title}</h1>
      ...
      <nav aria-label={title} className="flex gap-2 overflow-x-auto border-b border-pc-border px-5 py-4 sm:px-10 lg:hidden">
        {sections.map((section) => <a key={section.id} href={`#${section.id}`} className="shrink-0 rounded-lg border border-pc-border bg-pc-bg-secondary px-3 py-2 text-xs font-medium text-pc-text-secondary transition-colors hover:border-pc-accent-mid hover:text-pc-accent">{section.title}</a>)}
      </nav>
      ...
      <div className="grid gap-10 px-5 py-12 sm:px-10 sm:py-16 lg:grid-cols-[15rem_minmax(0,1fr)] lg:px-16">
        <aside className="hidden lg:block">
          <nav className="sticky top-24 space-y-1 border-l border-pc-border pl-4">
            {sections.map((section) => <a key={section.id} href={`#${section.id}`} className="block rounded-r-lg px-3 py-2 text-sm font-medium leading-6 text-pc-text-secondary transition-colors hover:bg-pc-accent/10 hover:text-pc-accent">{section.title}</a>)}
          </nav>
        </aside>

        <div className="min-w-0 space-y-4">
          {sections.map(({ id, title: sectionTitle, body, icon: Icon, link }, index) => (
            <article id={id} key={id} className="pc-card scroll-mt-24">
              <div className="flex gap-4">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center text-pc-accent">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm leading-7 text-pc-text-secondary sm:text-[0.95rem]">
                    {body}
                    {link && <PolicySectionLink link={link} />}
                  </p>
                </div>
              </div>
            </article>
