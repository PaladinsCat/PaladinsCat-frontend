  ...
  return <div className="space-y-2 lg:hidden">
    {players.map((player, index) => <article key={player.id} className="pc-mobile-panel overflow-hidden">
      <div className="flex min-w-0 items-center gap-3 p-3">
        ...
        <div className="min-w-0 flex-1"><Link href={`/players/${player.id}`} className="block truncate text-sm font-semibold text-pc-text hover:text-pc-accent"><PlayerName playerId={player.id} cheater={player.cheater} susCount={player.susCount ?? player.sus_count}>{player.name}</PlayerName></Link><div className="text-[10px] text-pc-text-muted">{player.region || t("common.fallback.unknownRegion")} · {value(player.totalMatches)} {t("generated.players.matches.9f3e924")}</div></div>
        {showSeverity && <span className={`shrink-0 rounded-full border px-2 py-1 text-[10px] font-semibold capitalize ${SEVERITY_STYLES[player.severity] || "border-pc-border text-pc-text-muted"}`}>{player.severity ? t(SEVERITY_LABEL_KEYS[player.severity] ?? "common.fallback.review") : t("common.fallback.review")}</span>}
      </div>
      <dl className="grid grid-cols-3 gap-px border-y border-pc-border/60 bg-pc-border/60 min-[420px]:grid-cols-6">
        ...
        ].map(([label, metric]) => <div key={label} className="bg-pc-bg-elevated px-2 py-2 text-center"><dt className="text-[8px] uppercase tracking-wide text-pc-text-muted">{label}</dt><dd className="mt-0.5 truncate font-mono text-xs font-semibold text-pc-text-secondary">{metric}</dd></div>)}
      </dl>
