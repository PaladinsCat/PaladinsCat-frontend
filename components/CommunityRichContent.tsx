...
function videoEmbed(url: string): VideoEmbed | null {
  try {
    ...
    if (host === "youtu.be") {
      ...
      return /^[A-Za-z0-9_-]{11}$/.test(id) ? { src: `https://www.youtube-nocookie.com/embed/${id}`, title: "YouTube video" } : null;
    }
    ...
    if (host === "vimeo.com") {
      ...
      return /^\d+$/.test(id) ? { src: `https://player.vimeo.com/video/${id}`, title: "Vimeo video" } : null;
    }
  } catch {
  ...
  return (
    <div className="space-y-4 whitespace-pre-wrap break-words text-pc-text">
      ...
      {parts.map((part, index) => {
        ...
        return <div key={`embed-${index}`} className="aspect-video overflow-hidden rounded-xl border border-pc-border bg-black shadow-lg"><iframe src={embed.src} title={embed.title} className="h-full w-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div>;
      })}
    </div>
  );
}
