...
function PlayerBuildRow({ player, fact, wins }: { player: MatchPlayerDetail; fact?: MatchFactPlayer; wins: boolean }) {
  ...
  return <div className={`grid grid-cols-1 gap-3 border-b border-pc-border/60 px-3 py-3 last:border-b-0 lg:min-w-[780px] lg:grid-cols-[240px_1fr_1fr] lg:items-center lg:gap-4 lg:px-4 ${wins ? "bg-emerald-400/[0.025]" : ""}`}>
    ...
    <div><div className="mb-1.5 text-[9px] font-semibold uppercase tracking-wider text-pc-text-muted lg:hidden">Talent &amp; cards</div><div className="flex flex-wrap items-center gap-1.5">{(fact?.talents ?? []).map(t => <Asset key={`talent-${t.talent_id}`} sources={[findIcon("talents", t.talent_id, t.talent_name), t.icon_url, t.fallback_icon_url]} alt={t.talent_name ?? "Talent"} tone="border-amber-400/40" />)}{(fact?.cards ?? []).map(c => <Asset key={`card-${c.card_id}`} sources={[findIcon("cards", c.card_id, c.card_name), c.icon_url, c.fallback_icon_url]} alt={c.card_name ?? "Loadout card"} level={c.card_level ?? undefined} tone="border-pc-accent/30" />)}</div></div>
    <div><div className="mb-1.5 text-[9px] font-semibold uppercase tracking-wider text-pc-text-muted lg:hidden">Purchased items</div><div className="flex flex-wrap items-center gap-1.5">{(fact?.items ?? []).map(i => <Asset key={`item-${i.slot}-${i.item_id}`} sources={[findIcon("items", i.item_id, i.item_name), i.icon_url, i.fallback_icon_url]} alt={i.item_name ?? "Item"} level={i.item_level == null ? undefined : i.item_level + 1} />)}</div></div>
  </div>;
}
...
export default function ItemsLoadoutsSection({ team1Players, team2Players, team1Wins, team2Wins, factMap }: Props) {
  ...
  return <section className="overflow-hidden rounded-xl border border-pc-border bg-pc-bg-elevated"><div className="flex flex-wrap items-end justify-between gap-3 border-b border-pc-border px-4 py-3"><div><h2 className="text-lg font-bold uppercase tracking-wide text-pc-text">Items &amp; Loadouts</h2><p className="mt-0.5 text-xs text-pc-text-muted">Talent and cards · purchased items</p></div></div><div className="overflow-x-hidden lg:overflow-x-auto"><div className="hidden min-w-[780px] grid-cols-[240px_1fr_1fr] gap-4 border-b border-pc-border bg-pc-bg-secondary/60 px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-pc-text-muted lg:grid"><span>Champion / player</span><span>Loadout</span><span>Items</span></div>{rows(team1Players, team1Wins)}<div className="flex items-center gap-3 bg-pc-bg-secondary/60 px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-pc-text-muted"><span className={`h-1.5 w-1.5 rounded-full ${team2Wins ? "bg-emerald-400" : "bg-red-400"}`} />Opposing team</div>{rows(team2Players, team2Wins)}</div></section>;
}
