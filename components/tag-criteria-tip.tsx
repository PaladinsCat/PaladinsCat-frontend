...
export default function TagCriteriaTip({ criteriaKey }: { criteriaKey: TranslationKey }) {
  ...
  return <aside className="pc-surface flex max-w-3xl items-start gap-2 rounded-lg border border-pc-border px-3 py-2.5 text-sm leading-6 text-pc-text-secondary" aria-label={t("moderation.criteria")}>
    <Info aria-hidden="true" className="mt-1 h-4 w-4 shrink-0 text-pc-text-secondary" />
    <p><span className="font-semibold text-pc-text">{t("moderation.criteria")}:</span> {t(criteriaKey)}</p>
  </aside>;
}
