      ...
      {loading && players.length === 0 ? <LoadingPanel compact /> : players.length === 0 ? (
        ...
        <div className={`space-y-4 transition-opacity ${loading ? "opacity-60" : "opacity-100"}`}>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {players.map((player) => (
              <Link key={player.id} href={`/players/${player.id}`} className="flex min-h-24 h-full flex-col gap-2 rounded-xl border border-rose-400/20 bg-pc-bg-elevated p-3 transition-colors hover:border-rose-400/40">
                <div className="flex min-w-0 items-start justify-between gap-2">
                  ...
                  <span className="w-fit shrink-0 rounded-full border border-rose-400/30 bg-rose-400/15 px-2 py-1 text-xs font-semibold text-rose-100">
                    {t("moderation.value1FlaggedMatches", { value1: formatNumber(player.masterFeedingCount) })}
                  </span>
                </div>
              </Link>
            ))}
