...
function CentralLogin() {
  ...
  return <div className="min-h-[60vh] flex items-center justify-center"><div className="w-full max-w-md bg-pc-bg-elevated rounded-lg border border-pc-border p-6 text-center">
    ...
    <OidcLoginForm returnPath={returnPath} />
...
function OidcLoginForm({ returnPath }: { returnPath: string }) {
  const { t } = useLocalization();
  return <form action="/api/auth/oidc/login" method="post">
    <input type="hidden" name="return" value={returnPath} />
    <button type="submit" className="w-full py-2.5 bg-pc-accent hover:bg-pc-accent-secondary text-white font-semibold rounded-lg transition-colors">{t("generated.auth.oidc.continue")}</button>
  </form>;
}
...
function LegacyLogin() {
  ...
  return <div className="min-h-[60vh] flex items-center justify-center"><div className="w-full max-w-md">
    ...
    <div className="bg-pc-bg-elevated rounded-lg border border-pc-border p-6 space-y-4">
      <OidcLoginForm returnPath={redirectPath} />
      <div className="flex items-center gap-3 text-xs text-pc-text-muted"><span className="h-px flex-1 bg-pc-border" /><span>{t("generated.auth.oidc.divider")}</span><span className="h-px flex-1 bg-pc-border" /></div>
      ...
      <p className="text-center text-pc-text-secondary text-sm">{t("generated.auth.donTHaveAnAccount")} <Link href="/auth/register" className="text-pc-accent hover:text-pc-accent-light transition-colors">{t("generated.auth.createOne")}</Link></p>
    </div>
  </div></div>;
}
