...
export default function RegisterPage() {
  ...
  useEffect(() => { formRef.current?.requestSubmit(); }, []);
  return <div className="min-h-[60vh] flex items-center justify-center"><div className="w-full max-w-md bg-pc-bg-elevated rounded-lg border border-pc-border p-6 text-center">
    ...
    <form ref={formRef} action="/api/auth/oidc/login" method="post"><input type="hidden" name="intent" value="create" /><button type="submit" className="w-full py-2.5 bg-pc-accent hover:bg-pc-accent-secondary text-white font-semibold rounded-lg transition-colors">{t("generated.auth.createAccount")}</button></form>
  </div></div>;
}
