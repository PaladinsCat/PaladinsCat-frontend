/**
 * Render one champion loadout card's ranked statistics.
...
/**
 * Render `/stats/loadouts/[name]/cards/[cardId]` using the canonical card-stat API.
 * refs: none
...
export default function ChampionCardDetailPage() {
  ...
  const selectedTalentId = parseMaybeNumber(searchParams.get("talentId"));
  const champion = STATIC_CHAMPIONS.find((entry) => championSlug(entry.name) === championSlug(name));
  const requestKey = `${champion?.id ?? 0}:${cardId ?? 0}:${selectedTalentId ?? "all"}:${lobbyTier.tierMin ?? "all"}:${lobbyTier.tierMax ?? "all"}`;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    if (!name || !cardId || !champion) return;
    ...
    Promise.all([
      ...
      fetchChampionCardDetail(champion.id, cardId, "ranked", selectedTalentId, {
  ...
  }, [cardId, champion, lobbyTier.tierMax, lobbyTier.tierMin, lobbyTierReady, name, requestKey, selectedTalentId, t]);
  ...
  function selectTalent(talentId: number | null) {
    if (!cardId) return;
    const base = `/stats/loadouts/${name}/cards/${cardId}`;
  ...
  if (!name || !cardId || !champion) {
    return <ErrorState title={t("generated.champions.cardStatisticsUnavailable")} message={t("generated.champions.invalidCardRoute")} />;
  }

  if (!lobbyTierReady || result.key !== requestKey) {
  ...
  if (error || !detail) {
    return (
      <div className="space-y-4">
        <ContextBackLink fallbackHref={`/stats/loadouts/${name}`} />
