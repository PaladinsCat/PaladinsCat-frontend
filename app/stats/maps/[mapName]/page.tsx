...
export default function MapDetailPage() {
  ...
  return <div className="space-y-6">
    ...
    <section className="relative overflow-hidden rounded-xl border border-pc-border bg-pc-bg-elevated"><SmartImage src={mapImagePath(map.name)} alt="" className="absolute inset-0 h-full w-full object-cover opacity-25" /><div className="absolute inset-0 bg-gradient-to-r from-pc-bg-elevated via-pc-bg-elevated/90 to-pc-bg-elevated/40" /><div className="relative flex flex-col gap-5 p-6 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="pc-heading pc-heading-lg text-pc-accent">{map.name.replace(/^Ranked\s+/, "")}</h1><p className="mt-1 text-sm text-pc-text-secondary">Ranked map distribution and map-specific meta.</p></div><div className="grid grid-cols-2 gap-5 text-right text-sm"><div><div className="text-[9px] uppercase text-pc-text-muted">Map Share</div><div className="font-bold text-pc-accent">{map.distributionRate.toFixed(1)}%</div></div><div><div className="text-[9px] uppercase text-pc-text-muted">Matches</div><div className="font-bold text-pc-text">{map.totalMatches.toLocaleString()}</div></div></div></div></section>
