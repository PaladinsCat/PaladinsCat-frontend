...
export default function MapsPage() {
  ...
  return <div className="space-y-6">
    ...
    {maps.length > 0 && <ContentFade className={`grid grid-cols-1 gap-4 transition-opacity sm:grid-cols-2 lg:grid-cols-3 ${loading ? "opacity-60" : "opacity-100"}`}>
      {maps.map((map) => <Link key={map.name} href={`/game/maps/${encodeURIComponent(map.name)}?scope=${encodeURIComponent(statsScope)}`} className="group overflow-hidden rounded-xl border border-pc-border bg-pc-bg-elevated transition-colors hover:border-pc-accent-mid"><div className="relative h-36 overflow-hidden bg-pc-bg"><SmartImage src={matchMapImagePath(map.name)} alt="" className="h-full w-full object-cover opacity-75 transition-transform duration-300 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-pc-bg-elevated via-pc-bg-elevated/10 to-transparent" /><h2 className="absolute bottom-3 left-4 right-4 truncate text-base font-bold text-pc-text transition-colors group-hover:text-pc-accent">{map.name.replace(/^Ranked\s+/, "")}</h2></div><div className="grid grid-cols-3 gap-2 p-3 text-center text-xs tabular-nums"><div><div className="text-xs uppercase text-pc-text-muted">{t("generated.stats.mapShare")}</div><div className="font-bold" style={{ color: getPercentageColor(map.distributionRate) }}>{formatPercent(map.distributionRate)}</div></div><div><div className="text-xs uppercase text-pc-text-muted">{t("generated.stats.avgTime")}</div><div className="font-medium text-pc-text">{duration(map.avgDurationSeconds)}</div></div><div><div className="text-xs uppercase text-pc-text-muted">{t("generated.stats.matches")}</div><div className="font-medium text-pc-text">{formatNumber(map.totalMatches)}</div></div></div></Link>)}
    </ContentFade>}
  </div>;
}
