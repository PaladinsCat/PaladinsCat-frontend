  ...
  return <div className="flex gap-2">
    {rows.map((row) => {
      ...
      return <div key={dimension} className="min-w-0 flex-1 rounded-lg border border-pc-border bg-pc-card/60 p-2 text-center">
        ...
        <div className="text-[9px] text-pc-text-muted">{percent(share)} share</div>
        <div className="text-[9px] text-pc-text-muted">{row.totalUses.toLocaleString()} purchases</div>
      </div>;
    })}
  </div>;
}
...
export default function ItemDetailPage() {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-xl border border-pc-border bg-pc-bg-elevated p-5" style={{ borderColor: overallQuality.borderColor }}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center"><img src={itemIcon(detail.itemName)} alt="" className="h-16 w-16 rounded-lg object-contain" /><div className="min-w-0 flex-1"><h1 className="pc-heading pc-heading-lg text-pc-accent">{detail.itemName}</h1><p className="mt-1 text-sm text-pc-text-secondary">Ranked item performance by purchase slot and final upgrade level.</p></div><div className="grid w-full grid-cols-2 gap-3 text-left sm:w-auto sm:grid-cols-3 sm:gap-5 sm:text-right"><div><div className="text-[10px] uppercase text-pc-text-muted">Win rate</div><div className="text-xl font-bold" style={{ color: overallQuality.color }}>{percent(detail.winRate)}</div></div><div><div className="text-[10px] uppercase text-pc-text-muted">Purchases</div><div className="text-xl font-bold text-pc-text">{detail.totalUses.toLocaleString()} <span className="text-sm font-medium text-pc-text-muted">purchases</span></div></div><div className="col-span-2 sm:col-span-1"><div className="text-[10px] uppercase text-pc-text-muted">Record</div><div className="text-sm font-semibold text-pc-text">{detail.wins.toLocaleString()}W / {detail.losses.toLocaleString()}L</div></div></div></div>
    </section>
    <section className="grid grid-cols-1 gap-4 lg:grid-cols-2"><div className="pc-card"><h2 className="pc-card-title">Purchase slot</h2><p className="mb-3 text-xs text-pc-text-secondary">Slots indicate the order and priority in which players purchase the item. Share is the portion of this item’s purchases made in each slot.</p><DimensionBars rows={detail.slots} label="Slot" total={detail.totalUses} /></div><div className="pc-card"><h2 className="pc-card-title">Upgrade level</h2><p className="mb-3 text-xs font-semibold text-pc-text-secondary">Levels indicate the effect of the item’s upgrade level on win rate. Share is the portion of observations ending at each level.</p><DimensionBars rows={detail.levels} label="Level" total={detail.totalUses} /></div></section>
