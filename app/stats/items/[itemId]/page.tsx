/**
 * Render /stats/items/[itemId] using `RouteSkeleton`.
 * refs: none
 */
"use client";
    ...
    <section>
      <div className="mb-4">
        ...
        <div className="flex flex-wrap items-center gap-2" role="group" aria-label={t("generated.stats.role")}>
          <button type="button" aria-pressed={selectedRole == null} onClick={() => setSelectedRole(null)} className={`pc-touch-target flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)] ${selectedRole == null ? "bg-pc-accent text-pc-bg" : "pc-surface text-pc-text-secondary hover:text-pc-text"}`}>{t("generated.champions.all")}</button>
          {CHAMPION_ROLES.map((role) => <button type="button" key={role.value} aria-pressed={selectedRole === role.value} onClick={() => setSelectedRole(selectedRole === role.value ? null : role.value)} className={`pc-touch-target flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)] ${selectedRole === role.value ? "bg-pc-accent text-pc-bg" : "pc-surface text-pc-text-secondary hover:text-pc-text"}`}><img src={role.icon} alt="" className="h-5 w-5" />{t(role.labelKey)}</button>)}
        </div>
      </div>
