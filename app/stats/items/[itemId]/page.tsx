  ...
  return <div className="flex gap-2">
    {rows.map((row) => {
      ...
      return <div key={dimension} className="min-w-0 flex-1 rounded-lg border border-pc-border bg-pc-card/60 p-2 text-center">
        <div className="text-[10px] text-pc-text-muted">{label === "Slot" ? "Buy" : "L"}{dimension}</div>
...
export default function ItemDetailPage() {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-xl border border-pc-border bg-pc-bg-elevated p-5" style={{ borderColor: overallQuality.borderColor }}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center"><img src={itemIcon(detail.itemName)} alt="" className="h-16 w-16 rounded-lg object-contain" /><div className="flex-1"><h1 className="pc-heading pc-heading-lg text-pc-accent">{detail.itemName}</h1><p className="mt-1 text-sm text-pc-text-secondary">Ranked item performance by purchase order and final upgrade level.</p></div><div className="grid grid-cols-3 gap-5 text-right"><div><div className="text-[10px] uppercase text-pc-text-muted">Win rate</div><div className="text-xl font-bold" style={{ color: overallQuality.color }}>{percent(detail.winRate)}</div></div><div><div className="text-[10px] uppercase text-pc-text-muted">Purchases</div><div className="text-xl font-bold text-pc-text">{detail.totalUses.toLocaleString()}</div></div><div><div className="text-[10px] uppercase text-pc-text-muted">Record</div><div className="text-sm font-semibold text-pc-text">{detail.wins.toLocaleString()}W/{detail.losses.toLocaleString()}L</div></div></div></div>
    </section>
    <section className="grid grid-cols-1 gap-4 lg:grid-cols-2"><div className="pc-card"><h2 className="pc-card-title">Purchase order</h2><p className="mb-3 text-xs text-pc-text-secondary">Rate is this item’s share of its observed purchases at each buy slot.</p><DimensionBars rows={detail.slots} label="Slot" total={detail.totalUses} /></div><div className="pc-card"><h2 className="pc-card-title">Upgrade level</h2><p className="mb-3 text-xs text-pc-text-secondary">Rate is this item’s share of observations ending at each level.</p><DimensionBars rows={detail.levels} label="Level" total={detail.totalUses} /></div></section>
        <section className="pc-card overflow-x-auto"><div className="mb-1 flex items-center justify-between"><h2 className="pc-card-title">Slot × level</h2><span className="text-[10px] text-pc-text-muted">Win rate · purchase count</span></div><p className="mb-4 text-xs text-pc-text-secondary">The complete breakdown shows how early an item was bought and how far it was upgraded in the same match.</p><table className="w-full min-w-[520px] text-center text-xs"><thead><tr className="border-b border-pc-border text-pc-text-muted"><th className="px-3 py-2 text-left">Purchase</th>{detail.levels.map((level) => <th key={level.level} className="px-3 py-2">L{level.level}</th>)}</tr></thead><tbody>{detail.slots.map((slot) => <tr key={slot.slot} className="border-b border-pc-border/50"><th className="px-3 py-3 text-left font-medium text-pc-text">Buy {slot.slot}</th>{detail.levels.map((level) => { const cell = matrix.get(`${slot.slot}-${level.level}`); if (!cell) return <td key={level.level} className="px-3 py-3 text-pc-text-muted">—</td>; const quality = getStatQuality(cell.winRate, 1, 1); return <td key={level.level} className="px-3 py-3"><div className="font-semibold" style={{ color: quality.color }}>{percent(cell.winRate)}</div><div className="text-[10px] text-pc-text-muted">{cell.totalUses.toLocaleString()}</div></td>; })}</tr>)}</tbody></table></section>
  </div>;
}
