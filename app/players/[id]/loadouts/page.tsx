...
export default function PlayerLoadoutsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    setError(null);
    try { setData(await fetchPlayerLoadouts(playerId)); }
  ...
  const refresh = async () => {
    if (!data || data.freshness.manualRefreshRemainingSeconds > 0) return;
    setRefreshing(true);
    setError(null);
    try { setData(await refreshPlayerLoadouts(playerId)); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "Could not refresh player loadouts."); }
  ...
  if (loading) return <LoadingPanel label="Loading player loadouts…" detail="Checking the local 24-hour saved-deck cache." />;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="text-right"><button type="button" onClick={refresh} disabled={refreshing || !freshness || freshness.manualRefreshRemainingSeconds > 0} className="rounded-lg border border-pc-border bg-pc-bg-elevated px-3 py-2 text-xs font-semibold text-pc-text transition-colors hover:border-pc-accent-mid hover:text-pc-accent disabled:cursor-not-allowed disabled:opacity-50">{refreshing ? "Refreshing…" : freshness && freshness.manualRefreshRemainingSeconds > 0 ? `Refresh in ${formatCooldown(freshness.manualRefreshRemainingSeconds)}` : "Refresh loadouts"}</button><p className="mt-1 text-[10px] text-pc-text-muted">Manual refresh has a 10-minute cooldown.</p></div>
      </div>

      {error && <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-200">{error}</div>}
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-pc-border bg-pc-bg-elevated px-4 py-3 text-xs"><span className="text-pc-text-secondary">{data?.loadouts.length ?? 0} saved deck{data?.loadouts.length === 1 ? "" : "s"} found</span><span className="text-pc-text-muted">{freshness?.refreshedAt ? `Cache updated ${formatLocalDateTime(freshness.refreshedAt)}` : "No cached loadout check yet"}</span></div>
      ...
      <div className="rounded-xl border border-pc-border/70 bg-pc-bg-secondary/50 px-4 py-3 text-xs text-pc-text-muted"><SmartImage src="/images/icons/Player_Loadouts_Icon.png" alt="" className="mr-2 inline h-5 w-5 align-middle" />Loadouts are stored locally for 24 hours. Opening this page does not repeatedly spend Hi-Rez API calls.</div>
    </div>
  );
}
