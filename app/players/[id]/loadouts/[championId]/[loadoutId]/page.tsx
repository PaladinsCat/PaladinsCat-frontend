...
export default function PlayerLoadoutDetailPage() {
  ...
  return <div className="space-y-6"><div><Link href={`/players/${playerId}/loadouts/${championId}`} className="mb-2 inline-block text-xs text-pc-accent hover:underline">← {champion.name} saved decks</Link><section className="relative overflow-hidden rounded-2xl border border-pc-accent/30 bg-gradient-to-br from-pc-bg-elevated via-pc-bg-elevated to-pc-accent/10 p-6"><div className="flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-3"><img src={getChampionIconSafe(champion.name)} alt="" className="h-14 w-14 rounded-xl object-contain" /><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-pc-text-muted">Choose your loadout</p><h1 className="text-2xl font-bold text-pc-accent">{loadout.loadoutName}</h1><p className="mt-1 text-sm text-pc-text-secondary">{champion.name} · five-card saved deck</p></div></div><span className="rounded-full border border-pc-accent/30 bg-pc-accent/10 px-3 py-1.5 text-xs font-semibold text-pc-accent">Level 15 total</span></div></section></div><section><h2 className="mb-3 text-sm font-bold text-pc-text">Deck cards</h2><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">{entries.map(({ cardId, level, card }) => <article key={cardId} className="overflow-hidden rounded-xl border border-pc-border bg-pc-bg-elevated"><div className="relative aspect-[0.72] bg-pc-bg"><SmartImage src={card?.iconUrl || "/images/icons/Player_Loadouts_Icon.png"} alt={card?.name || `Card ${cardId}`} className="h-full w-full object-cover" /><span className="absolute bottom-2 right-2 rounded-lg border border-white/20 bg-black/80 px-2 py-1 text-sm font-bold text-white">{level}</span></div><div className="p-3"><h3 className="truncate text-sm font-bold text-pc-text">{card?.name || `Card ${cardId}`}</h3><p className="mt-2 text-xs leading-5 text-pc-text-secondary">{cardDescription(card?.description, level)}</p></div></article>)}</div></section><div className="rounded-xl border border-pc-border bg-pc-bg-secondary/50 px-4 py-3 text-xs text-pc-text-muted">Saved loadouts are read from the local cache. Return to the champion&apos;s deck list to use the 10-minute-cooldown refresh option.</div></div>;
}
