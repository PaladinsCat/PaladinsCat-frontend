...
export default function PlayerLoadoutDetailPage() {
  ...
  useEffect(() => { let cancelled = false; fetchPlayerLoadouts(playerId).then((data) => { if (cancelled) return; setFreshness(data.freshness); setLoadout(data.loadouts.find((entry) => entry.id === loadoutId && entry.championId === championId) ?? null); }).catch((cause) => { if (!cancelled) setError(cause instanceof Error ? cause.message : t("generated.players.[id].loadouts.[championId].[loadoutId].page.couldnotloadthissaveddeck")); }); return () => { cancelled = true; }; }, [playerId, championId, loadoutId]);
