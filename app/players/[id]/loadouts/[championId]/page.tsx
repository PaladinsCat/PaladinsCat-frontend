...
export default function ChampionLoadoutsPage() {
  ...
  return <div className="space-y-6"><PlayersPageHeader title={<>{champion.name} {t("generated.players.loadouts")}</>} />{error && <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-200">{error}</div>}<div className="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-3">{decks.map((deck) => <Link key={deck.id} href={`/players/${playerId}/loadouts/${championId}/${deck.id}`} className="group overflow-hidden rounded-xl border border-pc-border bg-pc-bg-elevated transition-colors hover:border-pc-accent-mid hover:bg-pc-bg-secondary"><div className="flex items-center justify-between border-b border-pc-border px-4 py-3"><div className="min-w-0"><div className="truncate font-semibold text-pc-text group-hover:text-pc-accent">{deck.loadoutName}</div><div className="text-xs uppercase tracking-wider text-pc-text-muted">{t("generated.players.savedDeck.90cdf3b")}</div></div><span className="rounded-full bg-pc-accent/10 px-2 py-1 text-xs font-semibold text-pc-accent">{t("generated.players.view")}</span></div><div className="grid grid-cols-5 gap-1.5 p-3">{deck.cardIds.slice(0, 5).map((cardId, index) => { const card = cardsById.get(cardId); const level = deck.cardLevels[index] ?? 0; return <div key={`${cardId}-${index}`} className="relative aspect-[0.72] overflow-hidden rounded border border-pc-border bg-pc-bg"><SmartImage src={card?.iconUrl || "/images/icons/Player_Loadouts_Icon.png"} alt={card?.name || t("generated.players.cardValue1", { value1: cardId })} className="h-full w-full object-cover" />{level > 0 && <span className="absolute bottom-0 right-0 rounded-tl bg-black/80 px-1.5 py-0.5 text-xs font-bold text-white">{level}</span>}</div>; })}</div></Link>)}{decks.length === 0 && <div className="rounded-xl border border-dashed border-pc-border p-8 text-center text-sm text-pc-text-muted lg:col-span-2 xl:col-span-3">{t("generated.players.thisPlayerHasNoSaved")} {champion.name} {t("generated.players.loadouts")}</div>}</div></div>;
}
