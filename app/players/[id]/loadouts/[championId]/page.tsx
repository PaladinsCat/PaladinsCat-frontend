...
export default function ChampionLoadoutsPage() {
  ...
  const refresh = async () => { if (!data || manualRefreshRemainingSeconds > 0) return; setRefreshing(true); setError(null); try { const response = await refreshPlayerLoadouts(playerId); setData(response); setError(response.refreshError); } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not refresh player loadouts."); } finally { setRefreshing(false); } };
