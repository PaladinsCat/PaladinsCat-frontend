  ...
  return (
    ...
    >
      ...
      {selected && selection && (
        ...
        >
          <span className="text-xs font-medium text-pc-text-secondary">Level</span>
...
export default function CreateBuildPage() {
  ...
  useEffect(() => {
    ...
    fetchChampions({ limit: "200" })
      ...
      .catch(() => setError("Failed to load champions"))
      .finally(() => {
  ...
  useEffect(() => {
    ...
    loadBuildReferenceData(selectedChampion.id, championSlug(selectedChampion.name))
      ...
      .catch(() => {
        if (!cancelled) setError("Failed to load champion build references");
      })
      .finally(() => {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setError(null);
    if (!canSubmit) {
      setError("Choose 1 talent and exactly 5 cards totaling 15 points before saving.");
      return;
    }
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-pc-text-secondary">
            Pick a champion, then choose up to 4 items, 5 loadout cards totaling 15 points, and 1 talent.
          </p>
        </div>
      </div>
        ...
        <div className="pc-card space-y-4">
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <label className="space-y-2">
              <span className="text-sm font-medium text-pc-text-secondary">Build name</span>
              <input
                type="text"
                value={name}
                ...
                placeholder="Main ranked loadout"
                required
              />
            </label>

            <label className="space-y-2">
              <span className="text-sm font-medium text-pc-text-secondary">Champion</span>
              <select
                value={championId || ""}
            ...
            <label className="space-y-2">
              <span className="text-sm font-medium text-pc-text-secondary">Visibility</span>
              <select
                value={visibility}
          ...
          <label className="space-y-2 block">
            <span className="text-sm font-medium text-pc-text-secondary">Notes</span>
            <textarea
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              rows={3}
              className="w-full rounded-lg border border-pc-border bg-pc-bg-secondary px-3 py-2 text-pc-text placeholder-pc-text-muted focus:outline-none focus:ring-2 focus:ring-pc-accent/50"
              placeholder="Matchups, map notes, playstyle, or buy-order guidance"
            />
          </label>
        </div>

        {!selectedChampion && (
          <div className="pc-card text-sm text-pc-text-secondary">Select a champion to load cards and talents.</div>
        )}
          ...
          <>
            <section className="pc-card space-y-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <h2 className="pc-card-title">Items</h2>
                  <p className="text-xs text-pc-text-secondary">Current item store: 20 options, choose up to 4.</p>
                </div>
            ...
            <section className="pc-card space-y-4">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h2 className="pc-card-title">Talents</h2>
                  <p className="text-xs text-pc-text-secondary">{referenceData.talents.length} talents available, choose 1.</p>
                </div>
            ...
            <section className="pc-card space-y-4">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-xs text-pc-text-secondary">{referenceData.cards.length} champion cards, choose 5 and spend exactly 15 points.</p>
                </div>
                <div className="flex items-center gap-3 text-sm font-semibold">
                  ...
                  <span className={cardPointTotal === MAX_CARD_POINTS ? "text-green-400" : overCardBudget ? "text-red-400" : "text-yellow-300"}>
                    {cardPointTotal}/{MAX_CARD_POINTS} pts
                  </span>
                </div>
              </div>
