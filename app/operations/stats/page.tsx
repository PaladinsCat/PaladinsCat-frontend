...
function MetricCard({ label, value }: { label: string; value: number | string }) {
  return <div className="rounded-xl border border-pc-border bg-pc-bg-elevated p-4"><div className="text-xs text-pc-text-muted">{label}</div><div className="mt-1 text-2xl font-bold text-pc-text">{typeof value === "number" ? value.toLocaleString() : value}</div></div>;
}
...
export default function OperationsStatsPage() {
  ...
  useEffect(() => { fetchPublicOperationsStats().then(setData).catch((reason) => setError(reason instanceof Error ? reason.message : t("generated.operations.statsLoadFailed"))); }, []);
