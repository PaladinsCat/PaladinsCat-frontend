...
import { ContentFade, ErrorState, StableMetricValue } from "@/components/async-state";
...
export default function OperationsStatsPage() {
  ...
  if (error) return <div className="mx-auto w-full max-w-5xl px-4 py-8"><ErrorState title={t("operations.title")} message={error} /></div>;
  if (!data || !routeSettled) return <div className="mx-auto w-full max-w-5xl px-4 py-8"><RouteSkeleton variant="dashboard" /></div>;
  const { totalMatches, directMatches, recoveredMatches, incompleteMatches } = data.ingestCoverage;
